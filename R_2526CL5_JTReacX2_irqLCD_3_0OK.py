...
# 3.0 Simplificamos condicion irq y de salida:'pulsado' esta duplicado
# 3.0 intermitencia de preparacion
...
p_project = "Game Reaction time x 2 player: w. interruption + LCD + only 1 end var + inter in prep"
p_version = "3.0 with LCD based on 3.0 w/lcd"
...
def pulsador_quehacer(pin):    
    global tiempoReaccion, pulsadorMasRapido
    if pulsadorMasRapido == None:
        tiempoReaccion = ticks_diff(ticks_ms(), tiempo_start)
        print(pin) # para debug
...
# Espero a pasar de fase justo despues de pulsar, que cambia pulsadorMasRapido
# a un valor distinto de None de tipo Pin
while pulsadorMasRapido == None:
    pass
